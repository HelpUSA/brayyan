...
def ensure_ai_screening_table(db):
    db.execute(text('CREATE TABLE IF NOT EXISTS ai_screening_records (id INTEGER PRIMARY KEY AUTOINCREMENT, source_filename TEXT, record_key TEXT, pubmed_id TEXT, doi TEXT, title TEXT, year INTEGER, journal TEXT, a_decision TEXT, a_confidence REAL, a_labels TEXT, b_decision TEXT, b_confidence REAL, b_labels TEXT, comparison_status TEXT, conflict_priority TEXT, provisional_decision TEXT, human_review_needed INTEGER DEFAULT 0, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)')
...
def parse_csv_bytes(content):
    ...
    for line_no, raw in enumerate(reader, start=2):
        ...
        if not row.get('key') and not row.get('title'):
            warnings.append(f'Skipped row {line_no}: missing key and title')
            continue
        rows.append(row)
...
def store_rows(db, rows, filename=None):
    ...
    stmt = text('INSERT INTO ai_screening_records (source_filename,record_key,pubmed_id,doi,title,year,journal,a_decision,a_confidence,a_labels,b_decision,b_confidence,b_labels,comparison_status,conflict_priority,provisional_decision,human_review_needed) VALUES (:source_filename,:record_key,:pubmed_id,:doi,:title,:year,:journal,:a_decision,:a_confidence,:a_labels,:b_decision,:b_confidence,:b_labels,:comparison_status,:conflict_priority,:provisional_decision,:human_review_needed)')
    inserted = 0
    for row in rows:
        db.execute(stmt, {
            'source_filename': filename, 'record_key': row.get('key'), 'pubmed_id': row.get('pubmed_id'), 'doi': row.get('doi'), 'title': row.get('title'), 'year': as_int(row.get('year')), 'journal': row.get('journal'),
            'a_decision': row.get('A_decision'), 'a_confidence': as_float(row.get('A_confidence')), 'a_labels': row.get('A_labels'), 'b_decision': row.get('B_decision'), 'b_confidence': as_float(row.get('B_confidence')), 'b_labels': row.get('B_labels'),
            'comparison_status': row.get('comparison_status'), 'conflict_priority': row.get('conflict_priority'), 'provisional_decision': row.get('provisional_decision'), 'human_review_needed': as_bool_int(row.get('human_review_needed')),
        })
        inserted += 1
    db.commit()
    return inserted

def parse_and_store_csv(db, content, filename=None):
    rows, warnings = parse_csv_bytes(content)
    inserted = store_rows(db, rows, filename)
    return {'filename': filename, 'status': 'stored', 'rows_received': len(rows), 'rows_inserted': inserted, 'warnings': warnings, 'expected_columns': EXPECTED_COLUMNS}
