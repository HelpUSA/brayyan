...
async def article_summary(project_id: str = "1", db: Session = Depends(get_db)):
    ...
    total = db.execute(text("SELECT COUNT(*) FROM ai_screening_records")).scalar() or 0
    screened = db.execute(
        text("SELECT COUNT(*) FROM ai_screening_records WHERE coalesce(a_decision, '') <> '' OR coalesce(b_decision, '') <> ''")
    ).scalar() or 0
    included = db.execute(
        text("SELECT COUNT(*) FROM ai_screening_records WHERE lower(coalesce(provisional_decision, '')) LIKE '%include'")
    ).scalar() or 0
    conflicts = db.execute(
        text("SELECT COUNT(*) FROM ai_screening_records WHERE lower(coalesce(comparison_status, '')) LIKE '%conflict%')
    ).scalar() or 0
...
async def prisma(project_id: str = "1", db: Session = Depends(get_db)):
    summary = await article_summary(project_id=project_id, db=db)
    return {
        "project_id": project_id,
        "identified": summary["total"],
        "duplicates": 0,
        "screened": summary["screened"],
        "excluded": max(summary["screened"] - summary["included"], 0),
        "full_text_assessed": summary["included"],
...
async def metrics(project_id: str = "1", db: Session = Depends(get_db)):
    ensure_ai_screening_table(db)
    sql = text(
        "SELECT lower(a_decision) AS a, lower(b_decision) AS b "
        "FROM ai_screening_records "
        "WHERE coalesce(a_decision, '') <> '' AND coalesce(b_decision, '') <> ''"
    )
